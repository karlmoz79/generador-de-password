    ...
    checkAllBreachesBtn.addEventListener("click", async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                const breached = results.filter(r => r.breached === true);
        ...
        try {
            ...
            passwords.forEach(p => {
                ...
                tr.querySelector(".check-breach-btn").addEventListener("click", async () => {
                    try {
                        const res = await fetch(`/api/check-breach/${encodeURIComponent(p.website)}`);
                        const data = await res.json();
                        if (res.ok) {
                            if (data.breached) {
                                alert(`⚠️ ALERTA: La contraseña de ${p.website} ha sido comprometida.\n\nEsta contraseña aparece en ${data.count.toLocaleString()} filtraciones de datos.\n\nSe recomienda cambiarla inmediatamente.`);
                            } else {
